...
@section('content')
    @php
        ...
        $tender_methods = \App\Models\TenderMethod::all();
        $tender_statuses = \App\Models\TenderStatus::all();
        if (isset($tender)) {
            $units = \App\Models\OrganizationUnit::where('entity_id', $tender->official->unit->org->id ?? null)->get();
            $officials = \App\Models\Official::where('entity_unit_id', $tender->official->unit->id ?? null)->get();
        }
    @endphp
                ...
                <div class="wizard wizard-3" id="kt_wizard_v3" data-wizard-state="first" data-wizard-clickable="true">
                    ...
                    <div class="wizard-nav">
                        <div class="wizard-steps px-8 py-8 px-lg-15 py-lg-3">
                            ...
                            <div class="wizard-step" data-wizard-type="step" data-wizard-state="current">
                                <div class="wizard-label">
                                    <h3 class="wizard-title">
                                        <span>1.</span>{{ __('labels.tender_description') }}
                                    </h3>
                            ...
                            <div class="wizard-step" data-wizard-type="step" data-wizard-state="pending">
                                <div class="wizard-label">
                                    <h3 class="wizard-title">
                                        <span>2.</span>{{ __('labels.management_and_date') }}
                                    </h3>
                                    <div class="wizard-bar"></div>
                                </div>
                            </div>
                    ...
                    <div class="row justify-content-center py-10 px-8 py-lg-12 px-lg-10">
                        <div class="col-xl-12 col-xxl-7">
                            <!--begin: Wizard Form-->
                            <form class="form fv-plugins-bootstrap fv-plugins-framework" id="kt_form" method="post" action="{{ isset($tender) ? route('tender.update', $tender) : route('tender.store') }}" enctype="multipart/form-data">
                                @csrf
                                @if (isset($tender))
                                    @method('patch')
                                @endif
                                ...
                                <div class="pb-5" data-wizard-type="step-content" data-wizard-state="current">
                                    ...
                                    <div class="form-group fv-plugins-icon-container">
                                        ...
                                        <input type="number" class="form-control" name="amount" value="{{ $tender->amount ?? '0' }}" />
                                    </div>
                                    <div class="form-group fv-plugins-icon-container">
                                ...
                                <div class="pb-5" data-wizard-type="step-content">
                                    ...
                                    <div class="form-group fv-plugins-icon-container">
                                        ...
                                        <select class="form-control" name="" id="entity" style="width: 100%;">
                                            ...
                                            @foreach ($organizations as $org)
                                                <option value="{{ $org->id }}" @if (isset($tender)) @if ($org->id == ($tender->official->unit->org->id ?? null)) selected @endif @endif>{{ $org->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        <select class="form-control" name="" id="unit" style="width: 100%;">
                                            ...
                                            @if (isset($tender))
                                                @foreach ($units as $unit)
                                                    <option value="{{ $unit->id }}" @if ($unit->id == ($tender->official->unit->id ?? null)) selected @endif>{{ $unit->unit_name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="max_extended_process" class="form-control" value="{{ isset($tender) ? ($tender->max_extended_process ? $tender->max_extended_process->format('Y-m-d') : '') : '' }}">
                                    </div>
                                    <div class="form-group">
                                        <label>{{ __('labels.publication_date') }}</label>
                                        <input type="date" name="date_of_publication" class="form-control" value="{{ isset($tender) ? ($tender->date_of_publication ? $tender->date_of_publication->format('Y-m-d') : '') : '' }}">
                                    </div>
                                </div>
                                <!--end: Wizard Step 2-->
                                <!--begin: Wizard Step 3-->
                                    ...
                                    <div class="my-5">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="custom-file">
                                                ...
                                                <label class="custom-file-label" for="basement">{{ isset($tender) ? ($tender->basement ? __('labels.change_file_text') : __('labels.choose_file')) : __('labels.choose_file') }}</label>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="custom-file">
                                                ...
                                                <label class="custom-file-label" for="resolution">{{ isset($tender) ? ($tender->resolution ? __('labels.change_file_text') : __('labels.choose_file')) : __('labels.choose_file') }}</label>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="custom-file">
                                                ...
                                                <label class="custom-file-label" for="convocation">{{ isset($tender) ? ($tender->convocation ? __('labels.change_file_text') : __('labels.choose_file')) : __('labels.choose_file') }}</label>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="custom-file">
                                                ...
                                                <label class="custom-file-label" for="tdr">{{ isset($tender) ? ($tender->tdr ? __('labels.change_file_text') : __('labels.choose_file')) : __('labels.choose_file') }}</label>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="custom-file">
                                                ...
                                                <label class="custom-file-label" for="clarification">{{ isset($tender) ? ($tender->clarification ? __('labels.change_file_text') : __('labels.choose_file')) : __('labels.choose_file') }}</label>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="custom-file">
                                                ...
                                                <label class="custom-file-label" for="acceptance_certificate">{{ isset($tender) ? ($tender->acceptance_certificate ? __('labels.change_file_text') : __('labels.choose_file')) : __('labels.choose_file') }}</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex justify-content-between border-top mt-5 pt-10">
                                    ...
                                    <div>
                                        <button type="submit" class="btn btn-success font-weight-bold text-uppercase px-9 py-4" data-wizard-type="action-submit">{{ isset($tender) ? __('labels.save_changes') : __('labels.save') }}</button>
