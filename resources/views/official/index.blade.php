...
@section('content')
	<div class="panel panel-flat">
					<div class="panel-heading">
						<h5 class="panel-title">Official List</h5>				
					</div>
					...
					<div class="table-responsive">
						<table class="table table-togglable table-hover datatable datatable-header-basic">
							...
							<tbody>
								...
								<tr>
									<td>{{$loop->index+1}}</td>
									<td>{{$official->name}}</td>
									<td>{{$official->unit->unit_name}}</td>
									<td class="text-center">
										<a href="{{ route('official.edit', $official->id) }}" class="btn btn-defaut">
						...
						<ul class="fab-menu fab-menu-absolute fab-menu-bottom-right" data-fab-toggle="hover">
							<li>
								<a class="fab-menu-btn btn bg-success btn-float btn-rounded btn-icon" href="{{route('official.create')}}">
